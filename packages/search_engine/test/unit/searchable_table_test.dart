...
class _Row {
  const _Row({required this.id, required this.title, this.deleted = false});
...
void main() {
  group('searchableTable factory', () {
    test(
      'exposes the configured kind and watches via the supplied callback',
      () async {
    ...
    test('isDeleted defaults to false; honours the override when supplied', () {
      ...
      expect(tombstoned.isDeleted(const _Row(id: '1', title: 't')), isFalse);
    });

    test(
      'toGlobalSearch defaults to title/description/content from JSON',
      () async {
        final binding = searchableTable<GeneratedDatabase, _Row>(
          kind: 'rows',
          watch: (_, _) => const Stream.empty(),
    ...
    test(
      'toGlobalSearch default falls back to empty strings on missing fields',
      () async {
        final binding = searchableTable<GeneratedDatabase, _Row>(
          kind: 'rows',
          watch: (_, _) => const Stream.empty(),
        ...
        final result = await binding.toGlobalSearch(
          const PendingSearchItem(userId: 'u', kind: 'rows', id: '1', data: {}),
        );
    ...
    test('custom toGlobalSearch override is invoked', () async {
      ...
      final result = await binding.toGlobalSearch(
        const PendingSearchItem(userId: 'u', kind: 'rows', id: '7', data: {}),
      );
    ...
    test(
      'updatedAtOf throws by default and returns the override otherwise',
      () {
    ...
    test(
      'readSince throws by default and returns the override otherwise',
      () async {
        final base = searchableTable<GeneratedDatabase, _Row>(
          kind: 'rows',
          watch: (_, _) => const Stream.empty(),
